...
export default function SellerDashboard(props) {
  ...
  if (errorsItem) {
    for (let error of errorsItem) {
      ...
      if (error.msg.includes("Fooditems cannot")) fooditemsError = error.msg;
        ...
        <form>
          ...
          />
           <TextField
            name="fooditems"
            label="Food items"
            placeholder="Fish Curry,Veg"
            className={classes.textField}
            value={inputs.fooditems}
            onChange={handleInputChange}
            helperText={fooditemsError}
            error={fooditemsError ? true : false}
            fullWidth
          />
          <TextField
            name="description"
          ...
          />
          {
           ( image !==null && image !== undefined && image.length > 0) ? image.map((img,i)=>(<div style={{margin:"0 15px"}}><li>{img.name}</li></div>)):null
          }
